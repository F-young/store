...
def addUser():
    ...
    start = Bank.bank_addUser(account,User.getUserName(),User.getPassword(),Address.getCountry(),Address.getProvince(),Address.getStreet(),Address.getDoor())
    if start == 1:
        print("开户成功!")
        ...
        info = '''
            ...
            ---------------------------------
        '''
        print(info % (account,User.getUserName(),User.getPassword(),Address.getCountry(),Address.getProvince(),Address.getStreet(),Address.getDoor(),0,Bank.getBankName()))
    if start == 2:
        print("该用户名已经存在！！！")
    if start == 3:
        print("银行用户库已满！！！")
